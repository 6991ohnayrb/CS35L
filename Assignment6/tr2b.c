...
int main(int argc, char **argv) {
  ...
  int bool = 0;
  char c = getchar(); //gets first character
  while (c != EOF) { //continues until reach end of file
    bool = 0;
    for (i = 0; i < strlen(string1); i++) {
      if (c == string1[i]) { //checks if current char is in string1
        putchar(string2[i]); //if in string1, print corresponding string2 value
        bool = 1;
