...
//Made the following three variables global for multithreading purposes
int nthreads;
scene_t scene;
float scaled_color[width][height][3];

//Multithreading for the for loops originally in main
//This code was taken directly from the code in main
void* forloop (void* ptr)
{
    int* id = (int*)ptr; //cast
    ...
    //loop taking into account how many threads were created
    for( int px = *id; px < width; px += nthreads  )
    {
...
//    Does not handle the nthreads into 1 case
//    will be handled below with our call to multithread
//    if( nthreads != 1 )
//    {
...
//    }
    ...
    pthread_t *threading = malloc(nthreads * sizeof(pthread_t));
    
    int *info = (int *) malloc(nthreads * sizeof(int));
    
    for (int i = 0; i < nthreads; i++) *(info+i)=0;
    
    for (int i = 0; i < nthreads; i++) {
        pthread_create(threading + i, NULL, forloop, (info+i));
        *(info + i + 1) =* (info + i) + 1;
    }
    
    //Only join threads when complete
    for (int i = 0; i < nthreads; i++) pthread_join(threading[i], NULL);
    
    for (int x = 0; x < width; x++)
        for (int y = 0; y < height; y++)
            printf("%.0f %.0f %.0f\n", scaled_color[x][y][0],
                   scaled_color[x][y][1], scaled_color[x][y][2]);
